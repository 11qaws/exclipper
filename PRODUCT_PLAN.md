...
3. 모델은 오디오·대표 화면의 사건 해석 품질을 우선해 공식 Gemini 3.1 Pro Preview API 식별자 `gemini-3.1-pro-preview`와 실행 revision `3.1-pro-preview-07-2026`으로 고정한다. 응답은 `한국어 timestamp 대사`, `오디오·화면 기반 사건 단서`, `반응 단서`, `클립으로 검토할 이유`, `불확실한 점`의 닫힌 JSON schema를 사용한다. 후보 ID와 원본 절대 범위는 모델에 맡기지 않고 실행 snapshot에서 다시 주입한다.
