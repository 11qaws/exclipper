...
- 문서 상태: ExClipper 개인 편집 어시스턴트·상태/운영 모델을 확정한 초안 `0.3.11`
...
3. 모델은 실제 한국어 샘플에서 낮은 지연과 동일한 핵심 전사를 확인한 2026-05 안정 revision `gemini-3.1-flash-lite`로 고정한다. 응답은 `한국어 timestamp 대사`, `오디오 기반 사건 단서`, `반응 단서`, `클립으로 검토할 이유`, `불확실한 점`의 닫힌 JSON schema를 사용한다. 후보 ID와 원본 절대 범위는 모델에 맡기지 않고 실행 snapshot에서 다시 주입한다.
...
7. Pages Worker는 `{ audioBase64, candidateDurationMs }`만 중계에 넘기고 중계가 고정 prompt/schema를 만든다. 영상 프레임, 원본 파일명, 채팅 원문·집계, 기존 후보 점수·설명은 요청 계약에 없다. 한 후보 PCM과 Base64 요청은 완료·실패·취소 직후 참조를 해제하고, PCM은 0으로 덮는다.
...
- 원본 전체·영상 프레임·채팅을 Gemini 분석이나 서버 렌더 대상으로 삼지 않으며, 정밀 분석 입력은 빠른 분석이 고른 후보 오디오로 제한
