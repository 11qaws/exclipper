...
- 자동 선분석 대상은 YouTube `@AmorettoVODs`, `@eureka_history`, `@SENAREPLAY`, `@kokotorori`, `@망징-b1t` 다섯 source다. source마다 canonical channel ID와 저장 namespace를 고정하며 아모레또·유레카·세나·망징이는 long-form playlist, Shorts·커버가 섞인 코코는 완료된 live-stream playlist만 읽는다. 공식 playlist Atom feed는 증분 발견의 기준이며 각 entry가 반복하는 완전한 channel ID만 영상 소유권의 근거로 사용한다.
- 다섯 catalog는 서로 격리하지만 heavy 예약 실행의 처리 예산은 합쳐서 최대 2개 영상이다. 30분마다 lightweight Atom RSS+catalog preflight만 먼저 실행해 새 업로드를 `discovered`로 영속화하고, 실제 신규·context retry·review retry·`context-ready`의 review 누락·fingerprint 복구가 있을 때만 yt-dlp·WARP·AI가 필요한 heavy 준비를 연다. 수동 dispatch는 진단·정확 재시도를 위해 항상 heavy로 실행한다. heavy run은 source 순서를 회전하고 각 source의 첫 due 영상을 우선 배분해 한 채널의 backlog가 다른 채널을 굶기지 않으며, 영구적인 한국어 자막 부재 retry는 fresh·transient 작업 뒤로 보내고 한 run에서 최대 하나만 다시 시도한다.
