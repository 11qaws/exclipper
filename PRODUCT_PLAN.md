...
- 자동 선분석 대상은 YouTube `@AmorettoVODs`, `@eureka_history`, `@SENAREPLAY`, `@망징-b1t` 네 source다. source마다 canonical channel ID와 저장 namespace를 고정하며 모두 long-form playlist를 읽는다. 코코 개인 채널은 활성 source registry에서 제외해 예약 분석·링크 매칭·준비본 화면에 나타나지 않는다. 공식 playlist Atom feed는 증분 발견의 기준이며 각 entry가 반복하는 완전한 channel ID만 영상 소유권의 근거로 사용한다.
- 네 catalog는 서로 격리하지만 heavy 예약 실행의 처리 예산은 합쳐서 최대 2개 영상이다. `channel-preanalysis-scan.yml`이 매시 17분·47분의 서로 독립된 schedule에서 Atom RSS+catalog preflight만 실행하고, due 작업을 `queue: max`인 단일 heavy 실행열에 추가한다. 긴 분석이 진행 중이어도 다음 스캔은 막히지 않으며 heavy run은 한 번에 하나씩, 배정된 영상은 그 run 안에서 순차 실행한다. 최대 2개는 최소 동시 실행 수가 아니라 한 작업의 bounded 배정 상한이다. heavy 완료 직후 self-drain 스캔에서 다음 재시도가 2분 이내라면 그 정확한 시각까지만 가벼운 runner가 기다린 뒤 재판정해, 수십 초 차이로 작업이 끊기는 경계를 닫는다. 그보다 긴 backoff는 30분 schedule이 맡는다. 자동 due와 review queue에는 게시 시각이 현재로부터 정확히 7일 이내인 영상만 들어가고 source 안에서는 최신 게시 순으로 처리한다. source 회전으로 채널 간 기아는 막는다. Atom만으로 자막 유무를 확정할 수 없으므로 heavy 진입 직후 고정 yt-dlp로 한국어 수동·자동 자막을 확인하고, 둘 다 없으면 AI·예약 ASR을 호출하지 않은 `caption-pending` checkpoint로 남겨 7일 안에서만 재확인한다. 7일이 지난 항목은 catalog 기록은 보존하되 자동 transcript·context·review 선택에서 제외한다. 특정 source와 video ID를 지정한 수동 실행만 이 시간 제한과 자막 전용 제한을 우회해 진단 또는 예약 ASR 복구를 수행할 수 있다.
- 망징 source는 여러 방송을 한 영상에 묶는 `combined-replay`로 명시한다. 현재 exact 전체영상 일치는 사용할 수 있지만 로컬 하루치 원본을 합본 내부 구간에 자동 연결하는 subsequence 정렬은 아직 지원하지 않으므로, 이 경우 성공으로 가장하지 않고 로컬 분석으로 내려간다.
- GitHub Pages는 탭이 닫히면 주기 작업을 실행할 수 없으므로 발견부터 최종 후보 검증까지 예약 GitHub Actions가 수행한다. 작업은 YouTube의 bounded 480p 이하 분석 사본을 한 번 받아 전체 오디오 특징을 만든 뒤, 최대 12개 후보마다 30~60초 WAV와 서로 다른 JPEG 4장을 준비하고 전용 Worker에서 화면·오디오·대사·전체 방송 맥락을 함께 판정한다. 브라우저는 이 긴 작업을 반복하지 않는다.
- 예약 workflow는 `CHANNEL_PREANALYSIS_CONTEXT_PROXY_URL`과 `CHANNEL_PREANALYSIS_CONTEXT_TOKEN`을 시작 전에 모두 요구하고, 하나라도 없으면 성공처럼 보이는 전사 전용 실행을 만들지 않고 fail-closed한다. 두 값이 준비되면 `transcript-ready -> context-ready -> review-ready`가 자동으로 이어진다. 같은 전용 Worker의 `/v1/broadcast-context`와 `/v1/candidate-insights`를 사용하며 provider key는 Worker Secret에만 둔다.
- 후보의 16kHz mono PCM16 WAV와 48KiB 이하 JPEG 4장은 예약 Node 작업이 하나의 bounded binary bundle로 만들고 private R2에 stream stage한다. R2 native checksum·정확한 길이·WAV/JPEG signature가 닫힌 뒤에만 짧은 signed ticket과 canonical context JSON을 Worker에 보내며, provider는 만료가 짧은 capability URL로 같은 bytes를 읽는다. 따라서 Worker JavaScript는 수 MiB Base64 JSON을 decode·hash·재직렬화하지 않는다. 유효한 ticket은 그대로 재사용하고 만료되면 새 transport ticket·body를 발급할 수 있지만, media SHA·candidate fence·canonical context로 만든 semantic digest와 provider operation은 고정한다. terminal replay가 같은 후보의 중복 유료 호출을 막는다.
- 예약 작업은 `read-only prepare -> minimal write publish` 두 job으로 격리한다. dependency 설치·테스트·feed·`yt-dlp`·AI 호출은 repository write credential 없이 실행하고, publish는 검증 snapshot과 unchanged base SHA만 받아 catalog namespace에 반영한다. 후보별 완료 결과는 숨김 checkpoint 파일에 원자 저장하며, 어느 후보라도 실패하면 `review-ready`를 게시하지 않고 다음 run에서 완료 후보를 재사용해 실패 후보만 잇는다.
- 채널 영상의 주 상태는 `discovered -> metadata-ready -> transcript-ready -> context-ready -> review-ready`다. 시각 지문은 이 선형 상태를 막지 않는 후행 artifact lane이다. 지문 생성·검증이 실패하면 `retryable(fingerprint)`가 마지막 성공 `transcript-ready | context-ready | review-ready`를 가리키며, 확정된 자막·맥락·리뷰 artifact는 보존하고 지문만 다시 만든다.
...
- 자동 연결의 강한 근거는 명시적 YouTube video ID, 과거에 같은 catalog video에 등록된 exact 로컬 지문, 또는 현재 구현된 다중 시점 시각 지문 합의다. 제목·날짜·길이는 단일 영상 또는 최대 32개 duration cohort를 좁히는 단서일 뿐이며, 유일한 제목+길이 일치도 화면 합의 전에는 `probable`로만 표시한다.
...
- 예약 카탈로그는 공개 한국어 자막을 우선 사용하되 자막이 없으면 같은 free-R2
  transport로 90초 전 범위를 Groq Whisper Large V3 Turbo에 선전사한다. range별
  checkpoint를 catalog branch에 보존해 provider rate limit·workflow 중단 뒤 누락
  범위만 이어가며, 전사·전체 맥락·후보 AV artifact closure가 모두 완성된 영상은
  편집자가 연결하는 즉시 검토 화면으로 투영한다.
...
- 중계는 요청당 작업량을 작게 유지한다. 미디어 검증에 필요한 것은 컨테이너 헤더와 전체 길이뿐이므로 페이로드 전체를 디코드하지 않는다. 헤더 선행 바이트만 디코드하고 길이는 인코딩에서 산술로 계산한다. 메가바이트 단위 입력에 선형 1회 스캔을 넘는 검증을 두지 않는다.
- 모든 응답은 허용 origin의 CORS 헤더를 달고 나간다. 최상위 오류 경계가 없으면 Worker 실패가 플랫폼 기본 응답으로 대체되고, 브라우저는 그것을 CORS 위반으로 보고해 실제 원인을 가린다. 진단 가능성은 기능이다.
- 다만 자원 한도 초과로 런타임이 강제 종료되는 경우는 어떤 catch로도 잡을 수 없다. 따라서 오류 경계는 관측성 대책이고, 요청당 작업량 축소가 근본 대책이다.
- 결과가 비어 있는 이유를 하나의 문구로 합치지 않는다. `분석이 중단돼 판단하지 못함`과 `판단했으나 통과한 후보가 없음`은 사용자에게 완전히 다른 사실이며, 다음 행동도 다르다. 전자는 재시도 경로를, 후자는 보관된 빠른 후보 수와 다른 원본 경로를 제공한다.
- 실패로 인한 빈 결과를 방송의 문제로 표현하지 않는다.

## `0.3.46` 키보드 검토 루프와 앱 구조 분리

- 후보 판단은 마우스 전용이어서는 안 된다. `Space`, `←`/`→`, `Shift`/`Alt`+화살표, `A`, `R`, `Z`, `?`로 재생·이동·경계·판단·되돌리기를 마우스 없이 끝낼 수 있어야 한다. 기존 버튼은 발견성을 위해 모두 유지하고 `aria-keyshortcuts`만 더한다.
- 단축키는 `event.code`를 기준으로 한다. 한국어 UI에서 IME가 켜지면 `event.key`는 자모를 반환하므로 글자 단축키가 전부 무력화된다. 입력 요소와 IME 조합 중에는 단축키를 실행하지 않는다.
- 사용·빼기는 아직 판단하지 않은 다음 후보로 자동 이동한다. 판단 취소는 이동하지 않고, 남은 후보가 없으면 이동하지 않는다. 자동 이동은 반드시 알림과 되돌리기를 함께 제공하며 조용히 수행하지 않는다.
- 판단 수단은 근거보다 먼저 오고 화면에서 사라지지 않는다. 재생·클립·사용·빼기를 카드 상단에 sticky로 두어, 근거를 읽는 일과 결정하는 일이 순차적으로 강제되지 않게 한다.
- 같은 면책 문구를 한 카드에서 반복하지 않는다. `재생해서 확인` 안내는 카드당 한 번만 두고 하위 단서·cue는 이를 전제한다. 접근성 라벨의 안내는 예외로 유지한다.
- 컴포넌트 밖의 순수 코드는 `App.tsx`에 두지 않는다. 화면 타입, 한국어 문구, reason code 매핑, 타임라인 투영, 저장 변환, 브라우저 래퍼, 이동 규칙을 `src/app/`의 개별 모듈로 분리해 각각 테스트 가능하게 한다.
- 디자인 토큰은 별도 정리 작업으로 미루지 않는다. `styles/exclipper-foundation.css`가 타입·간격·elevation·motion·한글 줄바꿈을 소유하고, 새로 만드는 컴포넌트부터 이 토큰만 사용한다. 기존 룰은 각 작업이 자기가 건드리는 범위만 옮긴다.

## `0.3.45` 완전 검증 후보와 방송 요약 주석
...
- candidate insight 저장 schema는 `1.5.0`으로 올려 verification receipt와 새 판정 필드를 함께 보존한다. 이전 유료 결과와 섬네일은 계속 복구하지만 receipt가 없는 과거 결과를 완전 검증으로 가장하지 않고, 원본과 맥락이 준비되면 해당 후보만 다시 검증한다.

## `0.3.44` 화면 준비 큐와 화면 필수 후보 해석

- 하나의 `현재 진행` 패널을 모든 자동 단계가 공유한다. 1단계 `빠른 탐색`은 화면·오디오 탐색률, 2단계 `전체 맥락`은 대사 지도 수집과 맥락 해석의 연속 진행률, 3단계 `후보 종합`은 주제 지도 공개와 후보 정밀 검토율을 같은 위치·높이의 막대로 표시한다. 단계가 바뀔 때 별도 진행 카드를 추가하거나 막대를 숨기지 않는다.
- 시작 전 우상단에서 한국어/영어를 고를 수 있다. 분석이 시작되면 한 세션의 UI와 AI 서술 언어가 섞이지 않도록 선택을 잠그며, 대사 근거 원문은 편집 증거이므로 번역하지 않는다. 선택 언어는 후보와 전체 맥락 요청의 canonical input 및 cache identity에 포함한다.
- 방송 설명과 진행자 설명을 나란히 반복하지 않는다. 하나의 맥락 카드 안에서 `방송 흐름`은 시간순 사건·주제 전환을, `주 진행자의 진행 방식`은 반복 관찰된 말투·상호작용·반응 패턴과 근거 한계를 담당한다. 한국어 출력에서 의도하지 않은 한자·중국어 문자가 섞인 응답은 저장 전에 거부한다.
- AI는 아직 보지 못한 방송 위치를 최초 탐색 대상으로 고를 수 없다. 빠른 탐색이 후보 source range와 반응 정점을 확정하고, 결정적인 화면 추출 스케줄러가 각 범위의 정점 전후 네 위치를 고른다. 화면 추출 순서는 후보 점수·최종 순위가 아니며 결과는 candidate ID에만 귀속한다.
- 한 원본에 후보마다 새 video/Blob URL을 만들지 않는다. 한 개의 숨은 디코더와 canvas가 metadata를 한 번만 준비하고 후보 화면 묶음을 source-fenced 순서로 빠르게 생산한다. 준비된 묶음은 bounded 메모리 큐에 들어가며 최대 두 개 AI consumer가 다음 준비 완료 후보를 즉시 처리한다.
- 후보 AI 요청은 계획한 서로 다른 대표 화면 4장이 모두 준비된 경우만 허용한다. 한 장이라도 디코딩·JPEG 검증에 실패하면 오디오만으로 게임·장면·행동·인과를 추측하는 유료 요청을 보내지 않고, canonical fast 후보는 유지한 채 `대표 화면 묶음 미완성` gap으로 종료한다.
- AI에는 완성된 네 장의 화면 묶음 전체와 후보 오디오를 함께 보낸다. 반응 정점에 가장 가까운 화면을 같은 묶음에서 결정적으로 골라 타임라인 카드 섬네일로 즉시 표시하고, session thumbnail에도 같은 frame을 저장한다. AI 입력 화면과 사용자에게 보이는 대표 화면이 서로 다른 추출 실행에서 나오지 않게 한다.
- 등장인물 확인은 별도 유료 호출로 분리하지 않는다. 같은 멀티모달 응답에서 `identified | present-unidentified | none-present | insufficient-evidence` 중 하나를 반드시 반환하고, 사건·반응 요약에는 확인된 이름, 이름을 모르는 화면상 인물, 또는 `등장인물 없음`을 명시한다. 빈 배열만으로 부재와 식별 실패를 섞지 않는다.
- 외형 기준으로 이름을 붙일 때는 네 화면 중 서로 다른 두 장 이상에서 같은 고유 특징 묶음이 반복되어야 한다. 화면 이름·실제 호명은 한 시점 근거를 허용하지만 근거 frame과 상대 시각을 남긴다. `등장인물 없음`과 확인된 이름이 함께 나오거나, `identified`인데 식별 목록이 비어 있는 응답은 Worker 경계에서 거부한다.
- 출연진 기준은 방송 하나가 아니라 **채널 범위가 있는 버전형 카탈로그**로 확장한다. CHZZK 교환학생 메인 채널(`0385e1a232e51078bad18aef8479ab22`)은 세라 교수님을 진행자로 허용하지만, 아모레또·유레카·세나 아르벨·토로리 코코·망징이의 개인 채널에서는 세라 교수를 후보 명단에 넣지 않는다. 개인 채널의 주인은 강한 prior가 아니라 source-owner 단서이며, 실제 화면·이름·호명 근거가 없으면 해당 후보에 등장했다고 단정하지 않는다.
- 개인 채널의 여러 공개 영상은 향후 의상·표정·화면 배치가 달라져도 유지되는 고유 특징을 검증하는 reference set으로 사용할 수 있다. 운영 앱은 원본 영상을 다시 전송하거나 학습하지 않고, 검토된 특징 revision과 채널 ID만 서버 고정 카탈로그로 사용한다. reference revision 변경은 이미 저장한 유료 결과를 새 식별 결과로 가장하지 않는다.
- 최종 검토는 `타임라인 패널 + 떨어진 영상 카드 + 화면 아래 상세 카드` 세 덩어리로 나누지 않는다. 하나의 외곽 검토 작업면 안에서 왼쪽은 전체 방송 지도와 후보 목록, 오른쪽은 후보 이동·일시정지 영상·선택 후보 설명·사용/제외·근거를 위에서 아래로 연속 배치한다. 두 열은 같은 외곽 높이를 공유하고 오른쪽 긴 근거만 내부 스크롤하여, 왼쪽 후보 수 때문에 생긴 빈 공간과 영상·판정 사이의 긴 이동을 제거한다.
- 넓은 화면의 검토 열은 영상 아래에서 현재 후보의 핵심 사건, 등장인물 상태, 반응 정점, 주요 행동을 즉시 보여 준다. 고급 사건·반응 단서는 같은 열의 단계적 공개로 남기며, 별도 전체 폭 상세 카드를 다시 만들지 않는다. 좁은 화면에서는 `타임라인 → 영상 → 후보 판정`의 동일한 업무 순서로 한 열에 접는다.
- 전체 방송의 최초 분산 화면 위치도 향후 같은 추출 스케줄러가 결정해야 한다. AI가 준비된 seed 화면에서 의미·불확실성을 찾은 뒤에만 인접 범위를 추가 요청할 수 있다. 이번 버전은 후보 정밀 분석 큐를 먼저 완성하며, transcript 중심 전체 맥락을 화면을 본 것으로 과장하지 않는다.

## `0.3.43` 실시간 탐색 근거·안전한 검토 도크·모바일 분석 파이프라인

- 과거 체크포인트와 현재 API 계약의 개수 차이를 사용자 오류로 노출하지 않는다. 전사 원본은 그대로 보존하고, 전체 맥락 요청 직전에 인접한 source-fenced chapter만 결정론적으로 합쳐 1~144개의 bounded chapter로 만든다. 합친 범위의 시간·coverage·근거 문장은 유지하며 12시간 원본도 같은 계약으로 처리한다.
- 전체 맥락 모델의 최종 주제는 batch 응답 뒤에만 확정한다. 그 전에는 완료된 전사 탐색 셀을 `탐색 단서`로 즉시 공개하고, 셀을 누르면 실제 저장된 대사 요약·시간 범위·coverage를 inspector에서 연다. 탐색 단서를 최종 주제나 확정 후보로 가장하지 않는다.
...
- 원본 선택 완료 뒤의 첫 화면은 `업로드 폼 + 검사 영수증 + 떨어진 시작 버튼` 세 덩어리로 분리하지 않는다. 같은 높이와 같은 비중의 두 pane 안에서 왼쪽은 선택한 원본, 오른쪽은 그 원본으로 수행할 분석 설계를 보여 주며, 분석 시작 버튼도 오른쪽 작업 흐름의 마지막에 둔다.
- 왼쪽 pane은 파일 이름, 전체 길이, 형식, 크기와 원본 교체 동작만 유지한다. 이미 확인을 마친 상태에서는 링크 입력과 중복 검사 결과 카드를 숨겨 정보 우선순위를 명확히 한다.
- 오른쪽 pane은 실제 원본 길이를 00:00부터 끝 시각까지 투영하고 30분 간격 눈금을 그린다. 6시간과 12시간 원본에서도 모든 30분 구분선은 유지하되 글자 라벨만 각각 1시간·2시간 간격으로 줄여 겹침을 막는다.
- 분석 경로는 `방송 전체 훑기 → 맥락 구간 넓히기 → 여러 클립 후보 정리`로 한 번만 설명한다. 화면·오디오 준비 상태와 선택형 CHZZK 채팅 유무를 같은 pane에서 확인하고 바로 분석을 시작할 수 있어야 한다.
- 이 준비 시간축은 알려진 원본 길이만 표현하는 presentation projection이다. 아직 찾지 않은 후보, 주제, 잠재 점수를 만들어 내지 않으며 분석 시작 뒤 같은 source-time 축 위에 탐색 범위와 주제가 드러나는 결과 타임라인으로 자연스럽게 이어진다.
- 좁은 화면에서는 두 pane과 분석 경로를 단일 열로 접고 시작·끝 시각만 남긴다. 넓은 화면의 좌우 높이와 내부 카드 높이는 같은 grid track을 사용해 내용 길이에 따른 비대칭을 막는다.

## `0.3.41` 동적 맥락 탐색과 후보 공개 게이트

- 정보 타임라인은 장식용 기준선을 두지 않고 30분 time ruler를 유일한 x축으로 사용한다. 의미 주제는 사건·반응/성취·회수/흐름·전환/일반 맥락의 고정 색을 쓰며, 범위를 선택하면 저장된 요약·근거·확신·불확실성을 inspector에서 확인한다. 색만으로 의미를 전달하거나 hover에만 데이터를 숨기지 않는다.
- 전체 방송 서술은 600~1,000자 목표로 시간 순서·주제 변화·반복 소재·주요 사건을 보존한다. 별도의 주 진행 스트리머 프로필은 방송에서 관찰된 진행 역할, 말투와 채팅·게스트 상호작용, 관심사, 반응 패턴을 300~500자로 정리하고 근거와 한계를 함께 저장한다. 확인되지 않은 이름과 민감 신상은 추정하지 않는다.
- 후보 미니 카드는 100px의 동일 높이로 고정하고 제목·출처·두 줄 사건 설명의 슬롯을 공유한다. 공용 후보 이동 행 아래의 재생 pane과 근거 pane은 최대화 화면에서 1:1·동일 높이를 유지하며 긴 근거는 오른쪽 pane 내부에서만 스크롤한다.
- 편집자가 분석 절차 자체를 관리하게 만들지 않는다. 결과 작업 화면에는 하나의 실시간 진행 패널만 두고 `현재 진행 2/4`, 현재 작업, 한 줄 상태, 진행률을 중심으로 보여 준다. 전체 순서는 네 개의 작은 점으로만 확인하며 단계별 설명 패널은 기본 화면에서 제거한다.
- fast pass는 맥락을 어디서 볼지 알려 주는 잠재 신호이지 최종 클립이 아니다. 따라서 맥락·세부 분석 중에는 큰 후보 원, 후보 개수, 후보 카드, 편집기를 숨기고 흐릿한 점수 지형만 남긴다. 전체 방송 맥락과 의미 후보 위치 보정, 필요한 후보 화면·대사 검토가 끝난 뒤에만 하나의 종합 후보 집합을 공개한다.
- 긴 방송의 대사 표본은 source-time 순차 실행 대신 결정론적 분산 탐색으로 처리한다. 첫 셀은 방송 중후반의 황금비 위치에서 고르고 이후 가장 멀리 떨어진 미확인 셀을 골라 전역 coverage를 먼저 확보한다. 진짜 무작위가 아니므로 동일 입력의 재시도·복구·과금 순서가 재현된다.
- 한 셀에서 사건 표현, 인과·전환 표현, 질문·감탄, 비중립 감정 중 둘 이상이 확인되면 이미 예산에 포함된 인접 셀을 앞으로 당긴다. 로컬 확장 사이에 전역 probe를 끼워 한 사건에만 매몰되지 않는다. 음악 표기나 일반 진행 문장만으로는 확장하지 않으며 이 heuristic은 clip 승인이나 점수에 직접 쓰지 않는다.
- 타임라인은 `잠재 신호 → 최종 후보 대기/후보 → 맥락 탐색 셀 → 주제 흐름 → 의미 단서`를 한 화면에 배치한다. queued·active·complete·gap 셀을 구별하고 30분 눈금은 유지한다. 전체 맥락 모델은 현재 완성 응답을 반환하므로 주제 이름이 호출 도중 스트리밍된다고 표현하지 않고, 응답 뒤 source-fenced 주제를 분산 순서로 하나씩 시각화한다.
- 후보 공개 게이트는 canonical Candidate Ledger를 삭제하지 않는다. AI가 낮춘 후보도 편집자의 수동 확인과 기존 승인·경계를 보존하되, 지도에서는 작은 저강도 마커로 표현한다. 실패 시에는 저장된 fast 후보와 오류를 보존하고 명시적인 재시도 경로를 유지한다.
...
- 전체 맥락을 실행하지 않은 과거 결과, 저장 결과 확인 중, 실행 중, 실패, 구형 결과의 기능 미지원, 일부 근거만 확인, 완전한 근거에서 실제 0개를 서로 다른 상태로 표시한다. 미실행·미지원은 숫자 `0` 대신 `—`로 표시해 모델이 없다고 판정했다는 오해를 막는다.
- 타임라인의 coverage gap은 빗금 범위와 별도 범례로 표시한다. 이 범위는 사건 없음이 아니라 AI 근거 없음이며, 부분 표본 결과의 빈 주제·단서 레일도 `확인한 범위에서`라는 한계를 함께 밝힌다.
- GitHub 리뷰의 전체 App state-machine·Runtime Manifest 통합은 별도 migration으로 유지한다. 이번 범위는 기존 저장 schema와 canonical candidate ledger를 바꾸지 않는 복구·projection·표현 패치로 제한한다.

## `0.3.36` 주제 균형 의미 후보와 게임 방송 무후보 판정

- 주제별 고회수 탐색 결과는 최대 32개까지 비교 배심에 보낸다. 같은 퀴즈·토크·게임 형식 안에서도 대상, 갈등 원인, 스트리머 반응의 절정과 결론이 다르면 독립 사건이며, 긴 주제당 대표 하나로 축약하지 않는다.
- Qwen 3.7 비교 배심의 확정 선택은 그대로 보존하고 남은 내부 자막 정밀화 슬롯은 선택된 주제 사이에서 균형 있게 배분한다. 각 주제 안에서는 이미 선택한 시각과 멀리 떨어진 사건부터 확인해 한곳의 근접한 표현만 반복하지 않는다.
- 배심이 한 사건만 통과시켰으면 내부 후보가 폭증하지 않도록 선택+예비 확인을 최대 6개로 제한한다. 여러 독립 사건이 확인되면 단계적으로 최대 20개의 자막 구간을 저비용 텍스트 정밀화할 수 있다. 이 숫자는 사용자에게 보여 줄 후보 수가 아니다. 여기서 새로 제안하는 의미 후보와 한 번의 오디오·화면 상세 실행은 각각 최대 12개이며, 자막이 없는 유료 ASR 정밀화는 최대 4개와 `$0.03` reserve를 유지한다. 전체 canonical 원장은 GitHub 검토 원칙에 따라 AI가 삭제하지 않는다.
- 게임 방송 여부는 후보 하나의 과장된 문장이나 다른 날 방송 예고 한 줄로 정하지 않는다. 전체 챕터에서 게임 진행 근거가 반복되는지와 후보가 속한 로컬 주제를 함께 사용한다. 평범한 추락·좌표·채굴·제작·기지·시간 부족과 일반적인 채팅 장난은 높은 모델 confidence만으로 카드가 되지 않는다. 정확한 사과, 희귀 성취, 중대한 버그, 운영·금전·규칙·윤리 책임과 관련된 충돌, 장기 설정 회수는 예외로 다시 검토한다.
- 정상 자막 분석 한 건은 개요 1회, 주제 탐색 최대 4회, 비교 배심 1회, 자막 정밀화 최대 20회로 구성될 수 있으므로 Worker의 개인 맥락 요청 한도는 분당 30회다. 정밀화 호출은 최대 3개만 병렬로 보내며 최종 카드 12개와 전체 실행 `$1` 목표를 넘지 않는다.
- 역할 계획 `1.9.0`은 전체 맥락 단계의 실제 최대 호출 수를 26으로 표시하고, 예산 정책 `1.2.0`은 압축 맥락·자막 정밀화 텍스트 reserve를 실측 `$0.073543`보다 큰 `$0.08`로 둔다. 12시간 ASR·후보 영상·화면 챕터·고난도 판정·retry reserve를 모두 합쳐도 계획 상한은 `$1` 이하다.
- 세 실샘플은 서로 다른 계약으로 유지한다. 음식 토크는 칼국수·껍데기·두바이 초콜릿을 모두 내부 정밀화에 포함하고 오프닝 신호를 제외해야 한다. 구독 실수 방송은 정확한 사과·책임 인정·보상 흐름을 보존해야 한다. 마크 릴레이처럼 의미 있는 독립 사건이 없는 방송은 후보를 채우지 않고 0개로 끝나야 한다.

## `0.3.35` 전사 복구·대표 화면 안정화·닫힌 VTuber roster
...
> 분석 가능한 최대 12시간의 방송 원본과 선택적 자막·채팅 기록을 AI가 먼저 끝까지 훑고, 사람이 짧은 추천 후보만 재생해 30~60초 클립과 긴 하이라이트를 확정하는 개인 편집 보조 웹앱이다.
...
이 앱은 여러 사용자를 연결하는 서비스가 아니다. 계정·팀·초대·공동 편집·원격 프로젝트 DB·기기간 자동 동기화 없이 한 사람의 편집 작업만 다룬다. 같은 공개 Pages 주소를 여러 사람이 각자 열 수는 있지만 프로젝트·원본·후보를 서로 공유하는 기능은 없다. 공용 개발 지침의 일반적인 소규모 공유 서비스 기본값보다 이 프로젝트의 구체적인 개인용 범위가 우선한다.
...
|---|---:|---:|---:|---:|---:|
| 영상 파일(최대 12시간) | 브라우저/코덱 지원 시 가능 | 가능 | 가능 | 영상·음성 AI 가능 | 조건 충족 시 가능 |
...
- 로컬 파일: “파일을 고르면 AI가 먼저 훑어요. 추천 장면이 생기는 즉시 검토를 시작할 수 있어요.”
...
2. `라이브와 함께 기록하기` — 방송 시작 전 선택적으로 켜는 후속 기능. 공식 Session API 인증을 안전하게 처리하는 개인용 동반 도구가 필요하다. 공용 수집 백엔드는 만들지 않는다.
...
> 원본을 고르면 AI가 영상·음성·대사와 선택한 채팅 반응을 훑어요. 추천 장면만 확인하면 됩니다.

전면 요소:

1. 큰 버튼 `내 원본 고르기` — 완전한 AI 분석이 되는 권장 경로
...
7. 안내 문구 `로그인 없이 시작 · 지난 결과 이어보기 지원`
...
- `빠른 AI 분석 가능 · 대사 정밀 분석은 시간이 더 걸릴 수 있음`
...
큰 백분율 하나로 “AI 신뢰도 93%”를 보여 주지 않는다. 서로 다른 장르에서 잘못 보정된 숫자는 확신을 과장하기 쉽다. 대신 `근거 충분함`, `한 가지 신호만 있음`, `영상 미분석`, `조용한 장면이라 직접 확인 권장`처럼 사용자가 판단할 수 있는 상태를 표시한다. 승인·제외 피드백은 이 프로젝트의 후보 순서를 조정하는 데만 쓰며 전역 모델 학습 입력으로 사용하지 않는다.
...
5. `기록 끝내기`를 누르면 flush·checksum이 끝난 뒤 JSONL 위치와 `ExClipper에서 열기`를 보여 준다.
...
- 보존 범위와 삭제 방법을 가져오기 직전에 한 문장으로 설명한다.
...
| StreamSaver 패턴 | ExClipper 적용 | 초심자 이유 |
|---|---|---|
...
- 원본 StreamSaver 클래스에 새 기능 이름을 억지로 재사용하지 않고 ExClipper 전용 클래스는 `.rh-` 접두사를 사용
...
> 몇 시간짜리 영상도 짧은 조각으로 나눠 순서대로 확인합니다.

모델 준비:

> 첫 실행에는 AI 구성 요소를 받느라 시간이 조금 더 걸릴 수 있어요.

채팅 로그:

> 채팅을 추가하면 반응이 몰린 시간도 후보를 찾는 근거로 씁니다.

플랫폼 링크 모드:

> 링크만으로는 자동 분석 범위가 제한될 수 있어요. 가능하면 원본 파일을 골라 주세요.
...
- 채널별 게임명·인명·자주 쓰는 표현 사전을 프로젝트별로 선택 적용
...
3. Mediabunny 범위 디코드로 각 후보를 16kHz mono PCM16 WAV로 만들고, 배포 소유자의 Gemini 연결을 통해 한국어 전사·사건·반응·클립 이유를 요청한다.
4. 사용자는 API 키를 입력하지 않는다. 운영 키는 Cloudflare Worker Secret으로만 관리하고, 브라우저 요청은 후보 오디오와 후보 길이 두 필드로 고정한다.
...
`0.3.9`의 AI 수직 슬라이스는 한국어를 외국어처럼 잘못 받아쓰는 브라우저용 Whisper tiny를 제품 경로에서 제거하고, **AI가 먼저 고른 후보만 Gemini 3.1 Flash Lite로 정밀 분석**하는 기본 기능이다.

1. Pass A의 빠른 분석과 최대 12개 canonical 후보는 그대로 유지한다. Gemini는 후보를 새로 만들거나 점수·순위·구간·승인 상태를 자동 변경하지 않는다.
2. 사용자가 `후보 자세히 분석`을 누르면 Candidate Pass B Worker를 시작한다. 각 30~60초 후보를 점수 순서대로 하나씩 16kHz mono PCM16 WAV로 만들어 고정 중계 계약으로 처리한다.
3. 운영 후보 지각 모델은 안정 API 식별자 `gemini-3.5-flash`로 고정하고, `gemini-3.1-pro-preview`는 전체 맥락상 선택/제외가 어려운 최대 3개 후보의 재판정 역할로 제한한다. 응답은 `한국어 timestamp 대사`, `오디오·화면 기반 사건 단서`, `반응 단서`, `클립으로 검토할 이유`, `불확실한 점`의 닫힌 JSON schema를 사용한다. 후보 ID와 원본 절대 범위는 모델에 맡기지 않고 실행 snapshot에서 다시 주입한다.
...
6. 키는 `GEMINI_API_KEY` Cloudflare Worker Secret으로 설정하고 저장소, 정적 bundle, URL, 브라우저 저장소, 프로젝트 JSON, export, 오류 문구, 진단 로그에 넣지 않는다. 사용자는 키를 입력하지 않는다.
7. Pages Worker는 `{ audioBase64, candidateDurationMs, videoFrames?, castRosterId? }`를 중계에 넘기고 중계가 고정 prompt/schema를 만든다. `castRosterId`는 서버가 알고 있는 닫힌 enum이며 사용자 설명문을 받지 않는다. 전체 원본·원본 파일명·채팅 원문·집계, 기존 후보 점수·설명은 요청 계약에 없다. `videoFrames`는 후보당 최대 4장의 제한된 JPEG 대표 화면이다. 한 후보 PCM과 Base64 요청은 완료·실패·취소 직후 참조를 해제하고, PCM은 0으로 덮는다.
8. `401/403 중계 설정`, `429 할당량`, 네트워크·5xx, 잘못된 구조화 응답을 서로 다른 안전한 이유 코드로 구분한다. provider 원문 오류나 키는 사용자 문구에 복사하지 않는다. `5xx/408`만 짧은 지수 backoff로 최대 두 번 중계 재시도하고, 400·401·403·429와 앱 단계 전체 실행은 자동 반복하지 않는다.
...
10. 이번 슬라이스의 Gemini 결과는 세션 전용이며 persistence/export schema는 올리지 않는다. Gemini 실패는 fast-pass·반응 종류·재생·검토·내보내기를 막지 않고 이미 찾은 더 좋은 후보별 단서를 보존한다.

`0.3.9`의 성공 조건은 (a) 별도 키 입력 없이 후보별 한국어 대사와 오디오 기반 사건·반응 해석을 받고, (b) 사용자가 분석할 후보 수와 합계 시간을 실행 전에 알며, (c) 키가 정적 번들·브라우저 저장·로그에 포함되지 않고, (d) 모든 결과가 기존 fence 뒤에서만 화면에 들어오고, (e) Gemini가 실패해도 후보 검토와 내보내기가 그대로 동작하는 것이다.
...
원격 프로젝트 백엔드는 이 개인 편집 어시스턴트의 범위 밖이다. 한국어 정밀 분석에는 배포 Secret을 사용하는 stateless Gemini 중계만 둔다. 중계는 고정 Origin·요청 스키마·크기·횟수·시간 제한을 적용하고 프로젝트나 원본을 저장하지 않는다. Gemini 장애가 빠른 분석과 후보 검토를 막지 않도록 독립 실행 경계를 유지한다.
...
배포 health와 현재 브라우저 기능 상태를 각각 확인한다. build/schema/service-worker version, source 권한, 마지막 확정 checkpoint, runtime tier, 저장 공간, stale event 수를 표시한다.
...
- StreamSaver reference CSS + ExClipper 전용 CSS의 실제 앱 로드 순서 고정
...
- 초심자 사용성 시험과 빠른 분석/정밀 분석 단계 혼동 점검
...
- 스트리머 OAuth 동의와 Client Secret을 안전하게 처리하는 개인용 동반 도구
...
- 현재 계획 문서 버전은 `0.3.40`이다. `0.2.0`의 AI-first 방향에 개인용 제품 경계, 도메인별 lifecycle, 운영·복구 계약을 추가한 호환 가능한 구조 확장이다.
...
11. StreamSaver 원본 CSS는 불변 reference, ExClipper 수정은 별도 `.rh-` CSS에서만 한다는 점
...
- 후보 정밀 해석의 기본 provider와 실행 manifest는 `gemini / gemini-3.5-flash`다. 기존 배포·저장 결과를 다른 모델 결과로 가장하지 않고, provider를 바꾸기 전까지 후보 분석 결과와 비용·실패 특성도 그대로 유지한다. `gemini-3.1-pro-preview`는 최대 3개의 어려운 후보 재판정에만 사용한다.
- 후보 해석 대안으로 `qwen / qwen3.5-omni-flash`를 provider catalog에 등록한다. 이 모델은 현재 오디오와 대표 화면을 함께 보내는 30~60초 후보 계약에 맞는 복합 멀티모달 경로로 선택했다. `qwen3.6-flash`는 최대 2시간 화면 챕터, `qwen3.7-plus`는 재판정 폴백, `deepseek-v4-pro`는 압축된 전체 방송 문맥 역할이다. 아직 production transport는 `prepared`이며, 키와 Workspace ID만 넣었다고 자동 활성화하지 않는다. OpenAI 호환 응답·구조화 JSON·한국어 샘플에 대한 live smoke를 통과한 뒤 별도 patch에서 `active`로 올린다.
