...
7. Pages Worker는 `{ audioBase64, candidateDurationMs, videoFrames? }`를 중계에 넘기고 중계가 고정 prompt/schema를 만든다. 전체 원본·원본 파일명·채팅 원문·집계, 기존 후보 점수·설명은 요청 계약에 없다. `videoFrames`는 후보당 최대 4장의 제한된 JPEG 대표 화면이다. 한 후보 PCM과 Base64 요청은 완료·실패·취소 직후 참조를 해제하고, PCM은 0으로 덮는다.
