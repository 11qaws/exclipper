...
- 이전 0.9.2 수정은 일반 분석의 `.rh-review-workspace` 경로만 확인했고, 실제 배포
  스크린샷이 사용한 `PreparedReviewExperience > .prv > ReviewSurface` 경로에는
  `min-width: 760px`가 남아 있었다. 608px·663px 화면에서 내부 작업대가 740px로
  유지된 채 요약 2열과 근거 2열까지 겹쳐 적용되어 영상, AI 설명, 프레임, 대사가
  가로로 잘리고 한 글자씩 줄 바꿈되던 것이 직접 원인이었다.
- 820px 이하에서는 사전 분석 셸의 강제 최소 폭을 없애고, 왼쪽 세로 레일을 상단
  가로 도구막대로 바꿨다. 요약은 영상→판정→구간 조정→편집자 브리프, 근거는
  고정 플레이어→판정→4개 대표 프레임과 시간축→대사·맥락 순서의 한 열 작업
  흐름으로 전환한다. 620px 이하에서는 대사·맥락과 판정 버튼도 한 열/두 줄로,
  520px 이하에서는 대표 프레임을 2×2로 배치한다. 1024px부터는 기존 균형 2열을
  그대로 유지한다.
- 실제 사전 분석 툴바, 후보 12개, 긴 한국어 제목과 설명을 포함하는 전용 브라우저
  fixture를 추가했다. 390×844, 608×975, 663×968, 820×900에서는 좁은 폭 레이아웃,
  1024×768에서는 2열 전환을 직접 확인했다. 근거 탭 전환, 프레임 선택, 후보 이동도
  검사했으며 프레임을 눌러도 재생 버튼을 누르기 전에는 재생되지 않는다.
- 검증: TypeScript typecheck, 사용자 작업물 `.wrangler-dry-run-2/**`만 제외한 전체
  ESLint, 전체 Vitest 177파일·2,195개, 음성 등록 도구 9개, production build와
  Wrangler dry-run을 통과했다. 해당 생성물 폴더는 수정하지 않았다.
- 사용자 승인에 따라 앱 버전 `0.9.3` 배포 대상으로 확정했다.

## 2026-08-03 검토 화면 A안 — 편집자 브리프와 고정 근거 플레이어

- 검토 화면의 1280px 고정 작업대를 없애고, 1024×768부터 2560×1440까지 화면을
  실제 작업 공간으로 쓰도록 다시 배치했다. 요약은 작은 화면에서 영상:브리프 56:44,
  큰 화면에서 62:38이며, 근거 화면은 타임라인·대사·맥락과 고정 검토 플레이어를
  62:38 또는 68:32로 같은 작업면 안에 배치한다. 1440p에서는 가독성을 위해
  2320px까지만 확장한다.
- AI 설명을 한 문단으로 합치던 표시 모델을 사건, 스트리머 반응, 클립 가치, 방송
  주제, 전체 흐름 한 줄, 실제 대사로 분리했다. 대표 대사는 반응 요약을 재사용하지
  않고 정점에 가장 가까운 실제 transcript cue만 표시한다. 전체 맥락·인물 근거·
  불확실성은 근거 화면에 남기고, 빠른 탐색 기술 문구의 중복 표시는 제거했다.
- 클릭할 때만 열리던 340px 부유 플레이어와 App/키맵의 관련 상태를 제거했다.
  근거 화면에는 플레이어가 항상 존재하며, 프레임·대사·맥락을 누르면 자동 재생 없이
  해당 시점만 준비한다. 후보 전환 시 재생·근거 선택을 초기화하고 요약으로 돌아가므로
  첫 후보 이후 플레이어가 다시 열리지 않던 상태 불일치도 사라졌다.
- 브라우저에서 1024×768, 1280×720, 1920×1080, 2560×1440과 1000×600 최대 길이
  fixture를 확인했다. 문서 넘침 없음, 고정 플레이어 1개, 부유 카드 0개, 근거 클릭
  후 자동 재생 없음, 후보 전환 뒤 두 번째 후보에서도 근거 시점 이동 성공을 확인했다.
- 검증: TypeScript typecheck, 대상 ESLint, 전체 Vitest 177파일·2,195개, 음성 등록
  도구 9개, production build를 통과했다. 전체 ESLint는 사용자 작업물인
  `.wrangler-dry-run-2/**`만 ignore해 통과했으며 그 폴더는 수정하지 않았다.
- 배포 결정: 저장·Worker protocol·model manifest는 바꾸지 않는 UI-only patch로
  분류하고 앱 버전만 `0.9.0`에서 `0.9.1`로 올려 `main` Pages 배포 묶음에 포함한다.
...
### Qwen 연결과 예약 전사 파이프라인 폐쇄 · 2026-08-02

- **Qwen 운영 확인:** 새 key는 저장소나 설정 파일에 기록하지 않고 전경 Worker의
  `QWEN_API_KEY`와 예약 Worker의 `PREANALYSIS_QWEN_API_KEY` secret으로 등록했다. 두 secret은
  개행 없는 exact value로 다시 넣었으며 `/healthz`, `qwen3.6-flash`, `qwen3.7-plus`,
  `qwen3.5-omni-flash` 직접 호출과 실제 방송 맥락 smoke가 모두 200으로 끝났다.
- **자막 끝 경계:** YouTube JSON3의 마지막 cue가 metadata duration보다 늦어 정상 대사 전체가
  `INVALID_TRANSCRIPT`가 되던 문제를 producer 경계에서 수정했다. 원본 안에서 시작한 cue는
  끝까지만 clamp하고 원본 밖에서 시작한 cue는 제외한 뒤 digest를 만든다. 저장 bundle validator는
  계속 strict하다. Coco 실데이터는 1,620 event·44 chapter·검증된 digest로 정상 생성됐다.
- **자막 출처:** `.ko.json3` suffix를 수동 자막으로 간주하지 않는다. 동일 yt-dlp 호출의
  `subtitles`/`automatic_captions` metadata로 provenance를 결정하며, Coco처럼 자동 `ko`와
  `ko-orig`가 같은 bytes여도 `ko-orig`, `isAutoGenerated: true`로 보존한다.
- **ASR 다운로드 복구:** 16kHz mono 전사에 불필요한 `bestaudio` 대신 64kbps 이하를 우선한다.
...
### 준비된 검토 화면의 대형 화면 폭 고정 수정 · 2026-08-03

- **배포 대상:** 사용자 상시 자동 배포 승인에 따라 앱 버전 `0.9.4`로 올리고, 검증 후
  `main` 푸시와 GitHub Pages 실제 화면 확인까지 한 묶음으로 진행한다.

- **실제 원인:** 사전 분석 결과를 여는 경로는 `.rh-app > .prv`인데, 1101px 이상에서 모든
  `.rh-app`에 적용되는 태블릿 전시용 `display: grid`, `place-content: center`, `10vh 10vw`
  여백을 그대로 상속했다. `.prv`의 `width: 100%`는 자동 크기 grid track 안에서 계산되어
  1920px 화면에서도 작업면이 약 650px로 축소됐다. 기존 준비 화면 하네스는 `.rh-app` 부모를
  생략해 이 문제를 재현하지 못했다.
- **수정:** 준비된 검토 분기에 `rh-app--prepared-review` 화면 모드를 명시하고, 이 모드에서만
  전시용 grid와 여백을 해제해 `.prv`가 viewport 전체를 소유하도록 했다. 하네스도 실제
  `.rh-app > .prv` 구조를 사용하도록 고쳐 같은 회귀를 숨길 수 없게 했다.
- **브라우저 실측:** 1920px에서 검토 작업면 1872px, 1600px에서 1560px, 1440px에서
  1404px, 1024px에서 998px를 사용했다. 820px와 663px에서는 기존 단일 열 전환을 유지했다.
  전 크기에서 문서 가로 overflow와 미디어 자동 재생은 0이었다.
- **검증:** TypeScript, 변경 제품 파일 ESLint, production build, 음성 등록 도구 테스트 9개와
  전체 Vitest 177파일 2,195개가 통과했다. 첫 전체 실행에서 무관한 순열 테스트 하나가 5초
  제한을 0.45초 넘겼지만, 검증 서버를 정리한 단독 재실행은 9/9가 2.13초에 통과했고 이어진
  전체 재실행도 177/177파일이 통과했다.
  전체 ESLint는 기존 미추적 `.wrangler-dry-run-2/` 번들을 입력으로 읽어 실패했으며 해당
  사용자 산출물은 수정하지 않았다.

### 맥락 응답 JSON의 제한적 무손실 복구 · 2026-08-03

- **원인:** Qwen 맥락 분석이 내용 필드를 모두 생성했어도 API의 바깥 응답 봉투나 내부
  `message.content`에 앞뒤 설명문, Markdown 코드펜스, 후행 쉼표, 최상위 닫는 중괄호 하나 누락,
  또는 스키마 밖의 무해한 메타데이터가 있으면 `JSON.parse`·exact-key 경계에서 전체 응답을 버렸다.
  이 경우 이미 결제된 맥락 내용도
  `invalid-response`가 되어 제한된 대체 모델을 다시 호출하거나 최종 후보 0개로 끝날 수 있었다.
- **수정:** 전경·예약 Worker가 공유하는 맥락 응답 파서를 바깥 봉투와 내부 내용 양쪽에 적용하고,
  문자열과 escape를 인식하는 단일 패스 복구를 넣었다. 한 개의 JSON 객체만 추출하고,
  문자열 밖 후행 쉼표와 최상위 `}` 하나만
  보정한다. 모델 응답에서는 필수 필드와 모든 값의 기존 검증을 유지한 채 추가 키만 버리며,
  저장된 현재 스키마를 읽는 경계는 계속 exact-key로 닫혀 있다. 중첩 구조 누락·복수 객체·
  필수 필드 누락은 복구하지 않는다. 실제 Qwen 3.7 smoke에서 내용은 모두 유효하지만 챕터 보조
  분류 `kind`만 허용 목록 밖인 응답을 확인해, 비어 있지 않은 미지의 값은 스키마의 명시적
  포괄값 `other`로 낮추고 범위·제목·설명·중요도는 계속 엄격하게 검증한다.
- **진단·비용:** 복구 성공은 원문을 기록하지 않고 `X-ExClipper-Json-Recovered: true`와
  구조화 로그 한 건만 남긴다. 복구 뒤에도 실패하면 `X-Upstream-Parse-Stage`만 노출한다.
  1차 응답을 복구한 경우 대체 모델은 호출하지 않아 같은 맥락에 대한 중복 과금을 피한다.
- **운영 smoke:** 60초·챕터 1개·후보 1개의 현재 quota 계약 요청으로 전경 Worker를 검증했다.
  수정 전에는 Qwen 3.7 Plus가 `chapter-item-kind`에서 탈락해 Qwen 3.6 Flash로 전환됐지만,
  수정 후에는 3.7 Plus가 첫 세대에서 HTTP 200, fallback `false`, JSON recovered `true`로
  요약·후보 주석 1개·의미 챕터 1개를 직접 반환했다.
- **검증:** TypeScript, 전체 소스 ESLint warning 0(기존 사용자 산출물
  `.wrangler-dry-run-2/**`만 입력 제외), production build, 음성 등록 도구 9개, 전체 Vitest
  177파일 2,200개가 통과했다. 전경 Worker 474.26KiB·gzip 89.98KiB, 예약 Worker
  359.27KiB·gzip 69.61KiB로 두 Wrangler dry-run도 통과했다. 앱 버전은 `0.9.5`다.

### 메인 분석 데스크와 자동 큐 연속 처리 · 2026-08-05

- **정지 원인:** 30분 스캔은 한 번에 최대 두 영상을 무거운 작업으로 넘겼지만, 그 작업이
  끝난 뒤 다음 스캔을 즉시 깨우지 않았다. 예약 스캔이 지연되면 처리 가능한 항목이 남아도
  작업 수가 0이 됐다. 화면도 영상 수 대신 GitHub 실행 수를 표시해 한 실행이 두 영상을
  처리해도 `1`로 보였다.
- **연속 처리:** 무거운 작업이 카탈로그 체크포인트를 게시하면 가벼운 스캔을 즉시 다시 호출한다.
  스캔은 실제 배치 한도와 같은 최대 두 ID만 넘기며, 남은 즉시 처리 항목이 있으면 다음 직렬
  배치를 만들고 없으면 자연스럽게 끝난다. 30분 예약은 새 업로드 발견용으로 그대로 둔다.
- **메인 UI:** 파일·링크 입력을 같은 행, 준비된 분석·작업 상태를 같은 행에 놓는 2열 분석
  데스크로 재배치했다. 큐는 `처리 중 / 바로 처리 / 재시도 대기`를 영상 수로 구분한다. 기존
  태블릿 검토 셸의 `100dvh` 고정이 메인 화면 하단을 자르던 문제도 전용 front 모드로 분리해
  정상 문서 스크롤로 복구했다.
- **브라우저 검증:** 1600×900, 1440×900, 1024×768에서 가로 overflow 0과 동등한 행 높이를
  확인했고, 820px에서는 파일 → 링크 → 준비된 분석 → 큐 순서의 한 열로 전환됐다. 한국어 긴
  제목, 준비 결과 16개, 큐 9개의 현재 최대 데이터와 실행 중 영상 2개 표시를 확인했다.
- **배포 대상:** 앱 버전 `0.9.11`.

### 첫 화면 보조 도구와 큐 상태의 실제 의미 · 2026-08-05

- **배치:** CHZZK 채팅과 지난 분석을 별도 하단 행에서 제목 행의 44px 보조 도구로 옮겼다.
  파일·링크와 준비 목록·큐의 2열 구조는 유지하고, 데스크톱 준비 목록과 큐는 같은 330px
  외곽 높이를 사용한다. 긴 방송 목록과 큐 항목만 각 패널 안에서 스크롤한다.
- **상태 의미:** 겹치던 영상 집합을 `실행 중 작업 / 실행 대기 작업 / 시작 대기 영상 /
  재시도 예약`으로 분리했다. GitHub 실행 상태와 catalog 재시도 시각을 섞지 않으며,
  한 작업의 최대 두 영상은 동시에 두 개를 처리한다는 뜻이 아니라 순차 배정 상한임을 화면에
  직접 설명한다.
- **실제 정지 원인:** 세나 `2UjkCyKNGFA`는 2026-08-05 03:52 KST에 review 재시도가 due가
  됐지만 04:17 KST GitHub schedule event가 생성되지 않아 `start-pending`으로 남았다. 저장된
  맥락·후보 checkpoint에는 이상이 없었다. 수동 scan `30944056323`이 항목을 찾아 heavy run
  `30944109292`에 배정했고 실제 실행을 확인했다. GitHub가 고부하 때 예약 event를 누락할 수 있는
  경계를 UI에 숨기지 않고, 17분·47분 cron을 별도 schedule 항목으로 등록해 각 tick을 독립적으로
  관찰할 수 있게 했다. heavy 완료 뒤 self-drain도 계속 유지한다.
- **로컬 검증:** 큐 네 집합의 무중복 회귀를 포함한 Vitest 179파일·2,204개, 예약 파이프라인
  계약 63개, 음성 등록 도구 9개, TypeScript, ESLint warning 0, production build가 통과했다.
  1600×900과 1440×900에서는 CHZZK·지난 분석·두 입력·준비 목록·큐가 문서 스크롤 없이 한 화면에
  들어오고 가로 overflow가 없었다. 1024×768은 가로 overflow 없이 제목과 보조 도구가 겹치지 않는다.
- **배포 대상:** 앱 버전 `0.9.12`.
