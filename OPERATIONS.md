...
- Before treating a context response as `invalid-response`, the shared parser may recover only deterministic JSON packaging defects: surrounding prose or one Markdown fence, string-safe trailing commas, one missing top-level closing brace, and harmless extra object keys. A non-empty but unknown semantic-chapter `kind` is reduced to the schema's explicit `other` bucket while the chapter range, title, description, and salience remain mandatory. The parser never invents missing event text or IDs, never accepts an incomplete nested container, and never chooses between multiple JSON objects. A recovered success exposes `X-ExClipper-Json-Recovered: true`; a remaining parse failure exposes only the bounded `X-Upstream-Parse-Stage` class, including when the bounded fallback succeeds.
