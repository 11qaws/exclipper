...
- **UI·디자인 작업(특히 검토 화면)은 착수 전에 반드시 `docs/DESIGN_RULES.md`를 끝까지 읽고 그 계약을 따른다.** 요지: ① 모니터는 뷰포트, 태블릿은 정보 구성·인터랙션의 은유이며 물리적 기기(베젤·카메라·프레임·고정 기기 비율)를 그리지 않는다. ② 기존 DOM/CSS는 시각적 정답이 아니라 교체 대상이다 — 기능·상태·접근성·키보드만 보존하고, 기존 Review DOM·카드 구조·58:42·Review 전용 selector·시각 문법은 보존 대상이 아니다. ③ 기존 CSS 위에 override를 얹지 말고 격리된 새 루트/스타일을 만든 뒤 대체된 레거시 selector를 제거한다. ④ **코드보다 먼저** 삭제/병합/숨김 목록·정보 위계·두 개의 텍스트 와이어프레임·상태/모션 지도·브라우저 검증 기준을 제시하고 승인받는다. ⑤ 모호한 미감 요청("세련되게" 등)은 radius·shadow·타일이 아니라 여백·위계·삭제로 번역한다. build 성공은 디자인 검증이 아니며, 실제 브라우저와 긴 한국어 데이터로 확인한다.

## 2. 고정된 제품 정체성

- 이 제품은 **공유 서비스가 아닌 개인용 AI 편집 어시스턴트**다. 각 브라우저의 프로젝트·원본·후보는 독립적으로 유지하되, 하나의 배포 AI 중계 용량은 신뢰된 편집자 세션 최대 5개까지 공정하게 분배한다. 이는 계정·팀·공동 편집 기능을 뜻하지 않는다.
- 핵심 가치는 수시간 원본을 사람이 전부 보는 대신 AI가 먼저 하이라이트 후보를 찾고, 사람이 짧은 후보만 검토하게 하는 것이다.
- GitHub Pages 앱에서 원본 선택 → 빠른 분석 → 전체 맥락 → 완전 검증 후보 검토 → 메타데이터 출력까지 완주해야 한다. 후보 정밀 분석이 실패해도 candidate reservoir와 이미 결제한 근거·재시도 경로는 유지하되, 대사·맥락·네 화면·대표 썸네일이 미완성인 항목을 최종 후보로 가장하지 않는다.
...
- 후보 정밀 분석은 전체 원본 자체나 파일명·후보 점수·사람 검토 상태를 보내지 않고, 최대 12개의 30~60초 후보마다 오디오·서로 다른 대표 JPEG 4장·전체 방송 요약·source-fenced 앞/뒤 흐름·참고 대사·비식별 채팅 반응 요약만 고정된 bounded 스키마로 보낸다. 네 화면·대표 썸네일·전체 맥락·대사 중 하나라도 준비되지 않으면 최종 후보로 공개하지 않는다.
...
이 규칙은 실제로 반복된 실패를 막기 위한 것이다. 한 세션에서 이미 있는 모듈을
네 번 새로 만들려 했다 — `useReviewShortcuts`, `ShortcutHelpOverlay`,
`progressEstimate`, `thumbnailById`. 원인은 세 가지였고, 아래 네 항목이 각각을
막는다.

### 9.1 탐색과 설계는 영어를 기준으로 한다

코드 요소(모듈·함수·타입·필드·상태값)는 **영어 식별자를 그대로** 쓴다. 코드베이스
식별자가 전부 영어이고 `graphify` 그래프의 노드도 영어라, 한국어 개념어로 찾으면
걸리지 않는다. "도움말 오버레이"로 검색해서 `ShortcutHelpOverlay` 를 놓쳤다.

### 9.2 기록은 한국어로, 용어는 영어 그대로

문서·주석·설명은 한국어로 쓴다. 다만 그 안의 **코드 용어는 번역하지 않는다.**

- O: "`useReviewShortcuts` 가 키맵을 단독으로 소유한다"
- X: "검토 단축키 훅이 키맵을 소유한다"

번역한 용어는 **다음에 검색되지 않는다.** 다음 세션이 "검토 단축키 훅"으로 grep
하면 아무것도 안 나오고, 없다고 판단해 다시 만든다. 한국어는 설명의 언어이지
식별의 언어가 아니다.

### 9.3 "없음"은 전수 탐색으로만 결론낸다

한 파일이나 한 폴더를 보고 없다고 판단하지 않는다. `App.tsx` 에서 `keydown` 을
한 건 찾고 "키보드 처리가 없다"고 결론냈는데, 실제로는 별도 파일에 174줄짜리
훅이 있었다.

- 대상은 `src/` 전체로 잡는다
- 내용 검색보다 **`export` 전수 조회**를 우선한다 — 그 폴더의 표면 전체가
  빠짐없이 나오므로 추측에 의존하지 않는다
- 저장 필드처럼 함수가 아닌 것은 여러 저장소를 **모두** 확인한다. 하나만 보고
  "저장 안 됨"이라 결론내 `thumbnailById` 를 놓쳤다

### 9.4 만들기 전에 반드시 찾는다

새 모듈·상태·화면을 제안하기 전에 최소 한 번은 탐색한다. **"없을 것 같다"는
근거가 아니다.** `progressEstimate` 는 검색조차 하지 않고 없다고 가정했다.

`graphify` 를 먼저 쓴다.

- 설계 전: `graphify query "<영어 질문>"` 으로 찾는다
- 코드 변경 후: 그래프를 갱신한다. 코드만 대상이면 **API 비용이 0**이다
- `GRAPH_REPORT.md` 요약만 읽고 판단하지 않는다. 요약은 상위 몇 개만 보여 주므로
  노드가 있어도 안 보인다 — `graph.json` 을 직접 조회한다
- 그래프가 못 잡는 것: **저장 필드 단위**(예: `thumbnailById`). 그건
  `docs/MODULE_INDEX.md` 가 보완한다 — 설계 전에 그래프와 함께 읽는다
