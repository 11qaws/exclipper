...
export const STAGE_WEIGHTS: StageWeightTable = {
  /** 파일 메타데이터와 지문만. 원본이 6시간이든 20분이든 거의 같은 시간. */
  preflight: 2,
  /**
   * 방송 **전체**를 오디오·채팅·화면으로 훑는 유일한 구간이라 원본 길이에
   * 정비례한다. 6시간 원본에서 체감 대기의 절반 가까이가 여기다.
   */
  fastPass: 45,
  /** 이미 메모리에 있는 피크를 묶고 중복을 죽인다. 순수 계산이라 짧다. */
  seedClustering: 3,
  /** 디스크 쓰기. 짧지만 0 은 아니다 — 커밋을 기다린다. */
  commitFastResult: 2,
  /**
   * 방송 전역 맥락 탐색. 원격 호출이 여러 번 오가므로 네트워크에 달렸고,
   * 화면 페이즈 2 전체가 이것이다. 한때 이 일이 목록에 없어 진행축에서 통째로
   * 빠져 있었다.
   */
  broadcastContext: 18,
  /**
