 ...
 * 만들기 위한 표다.
 *
 * **이 값들은 실측이다** (2026-07-26, 2.25시간 원본 1회 · `app/stageTiming.ts` 의
 * 콘솔 표). 추정이던 시절의 값과 크게 달랐고, 특히 `broadcastContext` 는 18 로
 * 잡혀 있던 것이 실제 37 이었다. 표본이 하나뿐이므로 다른 길이·다른 기기에서
 * 다시 재면 조정한다 — 다만 **추정으로 되돌리지는 않는다.**
 *
...
export const STAGE_WEIGHTS: StageWeightTable = {
  /**
   * 실측 0% — 2.25시간 원본에서 0.0초. 파일 메타데이터와 지문뿐이다.
   *
   * 0 으로 두지 않고 1 을 주는 이유: 0 이면 이 스테이지가 도는 동안 막대가
   * 완전히 멈춘다. 실제로 순간이라 티가 안 나지만, 느린 디스크에서 몇 초가 걸리면
   * 그때 막대가 죽은 것처럼 보인다.
   */
  preflight: 1,
  /** 실측 36%. 방송 전체를 훑는 유일한 구간이며 원본 길이에 정비례한다. */
  fastPass: 36,
  /** 실측 0% — 메모리 안 계산. */
  seedClustering: 1,
  /** 실측 0% — 디스크 쓰기. */
   ...
   * 실측 **37%** — 가장 무겁다.
   *
   * 추정은 18 이었다. **두 배 넘게 틀렸고**, 그 오차가 이 표를 실측으로 바꾼
   * 이유다. 원격 왕복이 원본 길이만큼 쌓이는 구간이라 로컬 CPU 인 `fastPass`
   * 보다 무거울 수 있다는 것을 추정으로는 알 수 없었다.
   *
   * 다시보기 자막 조달이 겨냥하는 것이 정확히 이 구간이다
   * (`BOTTLENECK_AND_TRANSCRIPT_SOURCING_2026-07-26.md` §3).
   */
  broadcastContext: 37,
  /** 실측 27%. 후보 수에 비례하는 원격 호출. 추정 26 과 거의 같았다. */
  deepPass: 27,
  /** 실측 0% — 게이트와 순위 확정. */
  publication: 1,
};
